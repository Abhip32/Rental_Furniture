...
def index(request):
  ...
  if 'uid' in request.session:
      ...
      uid = request.session['uid']
      return render(request,"office.html",{'u':uid,'items':obj,'cart':cart,'total_sum':total_sum})
  return render(request,"office.html",{'items':obj})




def oaddToCart(request):
        ...
        if 'uid' in request.session:
          ...
          for items in St:
              if items.Product_Name==name:
                  ...
                  ps.save()
                  new_pro.save()     
          return render(request,"office.html",{'u':uid,'alert':"Order Added",'items':St,'cart':cart,'total_sum':total_sum})
        else:
            print("asd")
            return render(request,"office.html",{'u':uid,'alert':"Login First",'items':St})
